  ...
  return (
    <>
      ...
      {showCreate && (
        <>
          {/* Overlay */}
          <div className="fixed inset-0 bg-black/40 z-50" onClick={() => { setShowCreate(false); setAnhUrls([""]); }} />
          {/* Side panel */}
          <div className="fixed inset-y-0 right-0 w-1/2 bg-white shadow-2xl flex flex-col z-50">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">Điền đầy đủ thông tin để thêm tang vật</p>
              </div>
              <button onClick={() => { setShowCreate(false); setAnhUrls([""]); }} className="p-2 hover:bg-gray-100 rounded-lg">
                <X className="w-5 h-5" />
              </button>
            </div>
